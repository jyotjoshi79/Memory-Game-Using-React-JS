...
function App() {
  ...
  const submitPlayerName = async () => {
    try {
      const response = await fetch('/api/players', {
        ...
        body: JSON.stringify({ playerName, turns }),
      });
      if (response.ok) {
        console.log('Player name and turns submitted successfully!');
      } else {
        console.error('Failed to submit player name and turns.');
      }
    } catch (error) {
      console.error('Error submitting player name and turns:', error);
    }
  };
  ...
  useEffect(() => {
    if (fruits.every((fruit) => fruit.matched)) {
      ...
      submitPlayerName(); // Call the function to submit player name and turns
    }
